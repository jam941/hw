...
def list_to_linked_struct(lst):
    ...
    if (lst == []):
        ...
        return Node(lst[0], list_to_linked_struct(lst[1:]))
...
def filter_linked_struct(original, filter):
    ...
    if original == None:
        return None
    elif (contains_element(filter, original.value)):
        return filter_linked_struct(original.next, filter)
    else:
        return Node(original.value,filter_linked_struct(original.next, filter))
