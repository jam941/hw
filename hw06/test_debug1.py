...
def caesar(original, codeword):
    ...
    polar_shift = shift - 26
    for idx in range(len(codeword)):
        temp = ord(codeword[idx]) - ord(original[idx])

        if ord(codeword[idx]) - ord(original[idx]) != shift and ord(codeword[idx]) - ord(original[idx]) != polar_shift:
            return -1
    if(shift>=0):
